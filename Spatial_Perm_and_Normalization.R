...
CLQ_permutated_matrix_gen_caller <- function( filename){
...
significance_matrix_gen = function(iternum,
  ...
  df_full <- data.frame(matrix(ncol = 10, nrow = 0))
  colnames(df_full) <-c('sample','Cell_A' ,'Cell_A_num','Cell_A_count',
                        'Cell_B','Cell_B_num','Cell_B_count ',
                        'original_CLQ ','normalized_CLQ ',
                        'percentile')
  ...
  for (cA in 1:length(cell_types)){
    for (cB in 1:length(cell_types)){
      ...
      ### Normalized CLQ calculation:
      ref_level = mean(CLQ_array)
      max_level = max(CLQ_array) #nrow(cell_type_assignment_file) / counts_original[[cA]]
      normalized_CLQ = (original - ref_level ) / (max_level - ref_level)
      ...
      df_full[nrow(df_full) + 1,] = c(sample_to_check, cell_type_A, cA ,count_a,  cell_type_B, cB , count_b, original, normalized_CLQ, CLQ_matrix_Rank)
    }}
...
get_counts_caller = function(filename){
...
plot_gen = function(iternum, filename, list_of_matrices , CLQ_matrix_original){
  ...
  for (cA in 1:18){
    for (cB in 1:18){
      ...
      ref_level = mean(CLQ_array)
      max_level = max(CLQ_array) #nrow(cell_type_assignment_file) / counts_original[[cA]]
      original_normalized = (original - ref_level ) / (max_level - ref_level)
      
      if (is.nan(min(original,original_normalized,CLQ_array))) {
        # print(original)
        # print(original_normalized)
        # print(min(original,original_normalized,CLQ_array))
        next
      }
      png(file= file.path(paste0("./PA_figures_",  sample_to_check  ,"/cell_",cA,"_", cB,".png")),width=1000, height=600, res=200 )
      par(cex.axis=1.0)
      plot = hist(na.omit(CLQ_array), breaks=50, col='grey', main= paste0(cell_type_A,"\n", cell_type_B),
                  cex.main=0.7,
                  cex.lab=0.7,
                  cex.sub=0.7,
                  cex.axis=0.7, 
                  las=1, xlab='CLQ', 
                  xlim=c(min(original,original_normalized,CLQ_array)-5,
                         ifelse(original > max(CLQ_array),original+5,max(CLQ_array)+5)))
      abline(v=original, lwd=3, col="red")
      abline(v=original_normalized, lwd=3, col="blue")
      dev.off()
    }
  }
}
...
# filename_c = "CAFs1_Erlot_cell_type_assignment.csv"
# 
...
# ie. 
# files = list("TAFs1_Erlot_cell_type_assignment.csv",
#              "TAFs1_cell_type_assignment.csv",
#              "TAFs2_Erlot_cell_type_assignment.csv",
#              "TAFs2_cell_type_assignment.csv")
